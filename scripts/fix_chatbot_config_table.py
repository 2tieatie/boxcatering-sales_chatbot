        ...
        with engine.connect() as conn:
            ...
            if not result.fetchone():
                ...
                if 'text' in current_columns and 'prompt' not in current_columns:
                    logger.info("🔧 Found 'text' column instead of 'prompt' - fixing structure...")
                ...
                if 'language' not in current_columns:
                    ...
                    conn.execute(text("ALTER TABLE chatbot_configs ADD COLUMN language VARCHAR DEFAULT 'uk' NOT NULL"))
                
                if 'force_language' not in current_columns:
                    logger.info("➕ Adding 'force_language' column...")
                    conn.execute(text("ALTER TABLE chatbot_configs ADD COLUMN force_language BOOLEAN DEFAULT TRUE NOT NULL"))
                
                # Check if prompt and business_context columns exist
                if 'prompt' not in current_columns:
                    logger.info("➕ Adding 'prompt' column...")
                    conn.execute(text("ALTER TABLE chatbot_configs ADD COLUMN prompt TEXT NOT NULL DEFAULT ''"))
                
                if 'business_context' not in current_columns:
                    logger.info("➕ Adding 'business_context' column...")
                    conn.execute(text("ALTER TABLE chatbot_configs ADD COLUMN business_context TEXT NOT NULL DEFAULT ''"))
                ...
                for col_name in ['created_at', 'updated_at']:
                    if col_name in current_columns:
                        ...
                        if not col_info[3]:  # No default value
                            ...
                            conn.execute(text(f"ALTER TABLE chatbot_configs ALTER COLUMN {col_name} SET DEFAULT CURRENT_TIMESTAMP"))
