    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if 'language' not in existing_columns:
                ...
                conn.execute(text("UPDATE chatbot_configs SET language = 'uk' WHERE language IS NULL"))
                logger.info("✅ Existing records updated")
            
            if 'force_language' not in existing_columns:
                logger.info("🔄 Updating existing records to enforce language...")
                conn.execute(text("UPDATE chatbot_configs SET force_language = TRUE WHERE force_language IS NULL"))
                logger.info("✅ Existing records updated")
...
def main():
    ...
    if migrate_chatbot_config():
        ...
        logger.info("💡 You can configure language settings in the chatbot settings page.")
    else:
