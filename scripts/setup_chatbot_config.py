...
def setup_chatbot_config():
    ...
    try:
        ...
        if existing_config:
            logger.info(
                "✅ Chatbot configuration already exists, updating language settings..."
            )
            ...
            db.commit()
            logger.info(
                "✅ Updated existing configuration with Ukrainian language enforcement"
            )
        else:
...
def main():
    ...
    if setup_chatbot_config():
        logger.info("=" * 50)
        logger.info(
            "🎉 Success! Your chatbot is now configured with Ukrainian language enforcement."
        )
        logger.info(
            "💡 Try testing the chatbot - it should now respond only in Ukrainian!"
        )
    else:
