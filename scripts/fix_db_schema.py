...
def check_user_table_issues():
    ...
    try:
        ...
        if user_count > 0:
            ...
            logger.info(
                f"🔍 First user: ID={first_user.id}, Username={first_user.username}"
            )
            ...
            if first_user.created_at is None:
                logger.warning(
                    "⚠️  User created_at is None - this will cause 422 errors!"
                )
            if first_user.is_active is None:
                logger.warning(
                    "⚠️  User is_active is None - this will cause 422 errors!"
                )
    ...
    try:
        ...
        if conv_count > 0:
            ...
            if first_conv.created_at is None:
                logger.warning(
                    "⚠️  Conversation created_at is None - this will cause 422 errors!"
                )
    ...
    try:
        ...
        if order_count > 0:
            ...
            if first_order.created_at is None:
                logger.warning(
                    "⚠️  Order created_at is None - this will cause 422 errors!"
                )
...
def fix_database_schema():
    ...
    try:
        ...
        if users_without_created_at:
            logger.info(
                f"🔧 Fixing {len(users_without_created_at)} users without created_at"
            )
        ...
        if convs_without_created_at:
            logger.info(
                f"🔧 Fixing {len(convs_without_created_at)} conversations without created_at"
            )
            current_time = datetime.utcnow()
        ...
        if orders_without_created_at:
            logger.info(
                f"🔧 Fixing {len(orders_without_created_at)} orders without created_at"
            )
            current_time = datetime.utcnow()
...
def test_api_endpoints():
    ...
    try:
        ...
        logger.info(
            "💡 Now try accessing your dashboard again to see if 422 errors are fixed"
        )
