...
def create_default_system_configs(db):
    ...
    try:
        ...
        for config_data in default_configs:
            ...
            if existing_config:
                ...
                if (
                    ...
                    or existing_config.description != config_data["description"]
                    or existing_config.is_sensitive != config_data["is_sensitive"]
                ):
        ...
        db.commit()
        logger.info(
            f"✅ System configurations: {configs_created} created, {configs_updated} updated"
        )
        return True
...
def create_default_chatbot_config(db):
    ...
    try:
        ...
        if existing_config:
            logger.info(
                "ℹ️  Chatbot configuration already exists, updating language settings..."
            )
            ...
            db.commit()
            logger.info(
                "✅ Updated existing chatbot configuration with Ukrainian language enforcement"
            )
        else:
            ...
            default_config = ChatbotConfig(
                ...
                welcome_message=(
                    "Привіт! Я - Марічка, ваш AI-помічник з кейтерингу."
                    "Я можу допомогти вам з вибором меню, цінами, дієтичними вимогами та плануванням заходів. "
                    "Як я можу вам допомогти сьогодні?"
                ),
                business_context=(
                    "Ми є преміум сервісом кейтерингу в коробках, що спеціалізується на корпоративних заходах, "
            ...
            db.commit()
            logger.info(
                "✅ Created new chatbot configuration with Ukrainian language enforcement"
            )
...
def init_database():
    ...
    try:
        ...
        logger.info(
            "  • Default system configurations (OpenAI, Telegram, System, Security)"
        )
