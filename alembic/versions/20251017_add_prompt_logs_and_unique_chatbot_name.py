...
def upgrade() -> None:
    ...
    try:
        bind.execute(
            sa.text(
                "CREATE INDEX IF NOT EXISTS ix_prompt_logs_created_at ON prompt_logs (created_at)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX IF NOT EXISTS ix_prompt_logs_conversation_id ON prompt_logs (conversation_id)"
            )
        )
        bind.execute(
            sa.text(
                "CREATE INDEX IF NOT EXISTS ix_prompt_logs_config_id ON prompt_logs (config_id)"
            )
        )
    except Exception:
        pass
