...
def upgrade() -> None:
    ...
    try:
        existing_columns = {
            col["name"] for col in inspector.get_columns("chatbot_configs")
        }
    except Exception:
    ...
    with op.batch_alter_table("chatbot_configs") as batch_op:
        ...
        if "other_instruction" not in existing_columns:
            batch_op.add_column(
                sa.Column("other_instruction", sa.Text(), nullable=True)
            )
