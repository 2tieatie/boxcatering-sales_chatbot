...
async def create_user(
    ...
    if not role_service.can_create_role(current_user, user_data.role):
        raise HTTPException(
            status_code=403, detail="You cannot create users with this role"
        )
...
async def update_user(
    ...
    if "role" in user_data.model_dump(exclude_unset=True):
        ...
        if not role_service.can_create_role(current_user, new_role):
            raise HTTPException(
                status_code=403, detail="You cannot assign this role to users"
            )
        if not role_service.can_manage_user(current_user, user):
            raise HTTPException(status_code=403, detail="You cannot modify this user")
