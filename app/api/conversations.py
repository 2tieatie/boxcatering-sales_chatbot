...
async def update_conversation(
    ...
    if current_user.role == "manager":
        ...
        if (
            ...
            and update_data["assigned_to"] != current_user.id
        ):
            raise HTTPException(
                status_code=403,
    ...
    conversations = (
        ...
        .filter(
            Conversation.handover_state.in_(
                ["HANDOVER_PENDING", "HANDOVER_IN_PROGRESS"]
            )
        )
        .offset(skip)
        .limit(limit)
...
async def take_conversation(
    ...
    if conversation.handover_state != HandoverState.HANDOVER_PENDING:
        raise HTTPException(
            status_code=400, detail="Conversation is not pending handover"
        )
...
async def resolve_conversation(
    ...
    if conversation.assigned_to != current_user.id:
        raise HTTPException(
            status_code=403, detail="Can only resolve conversations assigned to you"
        )

    if conversation.handover_state != HandoverState.HANDOVER_IN_PROGRESS:
        raise HTTPException(
            status_code=400, detail="Conversation must be in progress to resolve"
        )
