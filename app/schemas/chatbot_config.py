...
class ChatbotConfigBase(BaseModel):
    ...
    welcome_message: str = Field(
        ..., min_length=1, description="Chatbot welcome message"
    )
    business_context: str = Field(
        ..., min_length=1, description="Business context description"
    )
    language: str = Field(
        default="uk", min_length=2, max_length=5, description="Language code"
    )
    force_language: bool = Field(
        default=True, description="Whether to enforce language compliance"
    )
    is_active: bool = Field(
        default=False, description="Whether this configuration is active"
    )
    chatbot_name: Optional[str] = Field(
        None, description="Public chatbot display name (e.g., Marichka)"
    )
    ...
    professional_style: Optional[bool] = Field(
        True, description="Maintain professional style"
    )
    suggestive_responses: Optional[bool] = Field(
        True, description="Offer follow-up suggestions"
    )
    manager_handover: Optional[bool] = Field(
        True, description="Enable manager handover"
    )
    ...
    response_timeout: Optional[int] = Field(
        30, description="Response timeout in seconds"
    )
    conversation_logging: Optional[bool] = Field(
        True, description="Enable conversation logging"
    )
    performance_analytics: Optional[bool] = Field(
        True, description="Enable performance analytics"
    )
...
class ChatbotConfigCreate(BaseModel):
    ...
    is_active: Optional[bool] = Field(
        False, description="Whether this configuration is active"
    )
    chatbot_name: Optional[str] = Field(
        None, description="Public chatbot display name (e.g., Marichka)"
    )
...
class ChatbotConfigUpdate(BaseModel):
    ...
    welcome_message: Optional[str] = Field(
        None, min_length=1, description="Chatbot welcome message"
    )
    ...
    professional_style: Optional[bool] = Field(
        None, description="Maintain professional style"
    )
    suggestive_responses: Optional[bool] = Field(
        None, description="Offer follow-up suggestions"
    )
    manager_handover: Optional[bool] = Field(
        None, description="Enable manager handover"
    )
    # fallback_message: Optional[str] = Field(None, description="Fallback message when AI can't understand")
    # handover_message: Optional[str] = Field(None, description="Message when transferring to manager")
    response_timeout: Optional[int] = Field(
        None, description="Response timeout in seconds"
    )
    conversation_logging: Optional[bool] = Field(
        None, description="Enable conversation logging"
    )
    performance_analytics: Optional[bool] = Field(
        None, description="Enable performance analytics"
    )
