(function () {
    ...
    const scriptTag = Array.from(document.getElementsByTagName("script")).find((s) => s.src.includes("i18n.js"));
    ...
    async function load(lang) {
        ...
        currentLang = target;
        try {
            const res = await fetch(homeLink + `/static/locales/${target}.json`, { cache: "no-store" });
